...
void test_Lexer_comments(void) {
    ...
    // Caso 2: Código con comentario al final
    {
        ...
        TEST_ASSERT_EQUAL(TOKEN_IDENTIFIER, Lexer_next_token(&lexer).type); // x
        TEST_ASSERT_EQUAL(TOKEN_EQUAL, Lexer_next_token(&lexer).type);    // =
        TEST_ASSERT_EQUAL(TOKEN_INTEGER, Lexer_next_token(&lexer).type); // 5
        TEST_ASSERT_EQUAL(TOKEN_EOF, Lexer_next_token(&lexer).type);
    }
    ...
    // Caso 3: Comentario seguido de código en nueva línea
    {
        ...
        TEST_ASSERT_EQUAL(TOKEN_IDENTIFIER, Lexer_next_token(&lexer).type); // x
        TEST_ASSERT_EQUAL(TOKEN_EQUAL, Lexer_next_token(&lexer).type);    // =
        TEST_ASSERT_EQUAL(TOKEN_INTEGER, Lexer_next_token(&lexer).type); // 10
        TEST_ASSERT_EQUAL(TOKEN_EOF, Lexer_next_token(&lexer).type);
    }
